...
#           <cmd> allNodeNumber runTimes oneTestTimeoutValue ipList;command;ipList:condition ... ipList;command;ipList:condition
#perl testSequence.pl 6 30 600 ;;1-6;ct003 ;;1-6;ct002 ;;1-6;ct001  1;
# sendcmt-10001-variable-789003-789001-assetType3-10-222:789001:789002:assetType1:10-ct003;; 4;
# sendcmt-10001-variable-789002-789003-assetType2-10-111:789003:789001:assetType3:10-ct002;; 1;
# sendcmt-10001-variable-789001-789002-assetType1-10-222:789002:789003:assetType2:10-ct001;;
#  > ct_2bc_3tx_4_24_1.log
...
sub processCommandOnLocal{
  ...
  for my $nodeFolder (@{$nodeipList}){
    my $folder = File::Spec->catfile($nodeFolder,"command");
    ...
    if (-1 == waitAndReplaceFile(File::Spec->catfile("$folder","command", $commandToExecute)))
    {
      return -1;
    }
  }
...
sub checkOnLocal{
  ...
  if (-1 == index ($condition, "log")){
    $folderOnCheckingNode = File::Spec->catfile($folderOnCheckingNode, "Blockchain_dump");
    @files = glob(File::Spec->catfile("$folderOnCheckingNode","*.dmp"));
  } else {# log
    $condition = substr($condition, 3);# remove log prefix
    @files = glob(File::Spec->catfile("$nodeFolder", "autoRun.log"));
  }
...
sub backUpFolder{
  ...
  for (my $i = 0 ; $i < $allNodeNumber; ++$i){
    my $dirname = ($i + 1);
    mkdir File::Spec->catfile($datetime, $dirname);
    say "[Main][INFO] Node back up folder has been created " . $dirname;
    my $c = "";
    if (-1 != index($OSNAME, "MSWin32")){
      say "MSWin32 system to backUpFolder";
      $c = "xcopy $dirname $datetime\\$dirname /s /e /y";;
    } elsif (-1 != index($OSNAME, "linux")){
      say "Linux system to backUpFolder";
      $c = "cp -rf $dirname $datetime//$dirname";
    } else{
      say "Unkonwn system, no command to backUpFolder";
    }
...
sub killChildThread{
  ...
  if (-1 != index($OSNAME, "MSWin32")){
    ...
    say "Unkonwn system, no command to kill process";
  }
  say "[Main][INFO] " . `$c`;
  #exit(0);
...
sub waitInitBlockchain{
  ...
  for (my $i = 0 ; $i < $allNodeNumber; ++$i){
    # $dirname =  1\Blockchain_dump
    my $dirname = File::Spec->catfile($i + 1, "Blockchain_dump");
    my $isInterBCOk = 0;
    my $isExterBCOk = 0;
    while (1 != $isInterBCOk || 1 != $isExterBCOk){
      sleep($checkInterval);
      my $dh;
      # $dh 为打开文件后，文件指针所指向的位置
      if (!opendir $dh, $dirname) {
        say "[Main][WARN] Can not open folder " . $dirname . " when at " . cwd;
        say "[Main][WARN] Try to enlong the sleep time";
        # next 语句将返回到循环体的起始处开始执行下一次循环 类似于 Java的continue
        next;
      }
...
sub startBlockchain{
  ...
  #  shift 如果省略了 ARRAY，那么该函数在子过程和格式的词法范围里移动 @_；
  my $id = shift;
  my $folderName = $id;
  ...
  #system() 函数
  # 你也可以使用 system() 函数执行 Unix 命令, 执行该命令将直接输出结果。 system( "ls -l")
  system($command);
}
...
sub processCommandStep {
  # #@_表示所有传递到本函数中的参数 ;;1-6
  my @commandList = @_;
  # 把数组的长度赋给了$processItemLength
  my $processItemLength = @commandList;
  waitInitBlockchain();
  # time  1970年1月1日起累计的秒数
  my $startEpoc = time;
  ...
  if (0 == $result) {
    ...
    say "*** End at $endEpoc [FAIlED]. Total time is " . ($endEpoc - $startEpoc);
  }
  backUpFolder();
  killChildThread();
}
...
sub main(){
  ...
  for (; $testRound < $runTimes; ++$testRound)
  {
   # $$运行当前Perl脚本程序的进程号
    ...
    # 将会出现错误的代码放入eval{}代码块中 类似于Java try{} catch{}
    eval{
