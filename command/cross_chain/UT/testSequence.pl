...
sub checkOnLocal{
  ...
  if (-1 == index ($condition, "log")){
    say "[MAIN][INFO] begin to check blockchain dump";
    $folderOnCheckingNode .= "\\Blockchain_dump";
    @files = glob("$folderOnCheckingNode\\*.dmp");
  } else {# log
    say "[MAIN][INFO] begin to check log file";
...
say "[UT][WARN] This is UT script. NOT for product scenario.";
#checkOnLocal(".\\", "log[Blockchain][internal][INFO]-Succeed-to-save-block");
checkParameter();
main();
