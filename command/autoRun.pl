...
sub main(){
  for (; $testRound < $runTimes; ++$testRound)
  {
    ...
    eval{
      say "set timeout value " . $timeoutValue;
      alarm $timeoutValue;
      oneTest();
    };
...
sub oneTest(){
  ...
  if (@ARGV < 5){
    say "[Main][ERROR] Not enough parameter. Format: allNodeNumber ACPParameterSettingNodeList initialConditionTransactionSendingNode runTimes timeoutValue";
    exit 1;
  }
...
sub waitFinish(){
  ...
  sleep(10);# 10 more seconds to ensure it has been finish
